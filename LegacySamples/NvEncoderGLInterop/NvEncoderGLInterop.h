...
class CNvEncoderGLInterop
{
public:
    CNvEncoderGLInterop();
    virtual ~CNvEncoderGLInterop();
...
protected:
    ...
    NVENCSTATUS                                          InitOGL(int argc, char **argv);
    NVENCSTATUS                                          AllocateIOBuffers(uint32_t uInputWidth, uint32_t uInputHeight);
    NVENCSTATUS                                          ReleaseIOBuffers();
    NVENCSTATUS                                          FlushEncoder();
    void                                                 TransferToTexture(uint32_t tex, uint32_t pitch, uint32_t height);
